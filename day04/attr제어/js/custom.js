$(document).ready(function(){
    ...
    */
    ...
    /*
        ...
        4. 해당 img태그의 src값을 위에서 찾은 변수로 변경
    ...
    */
    ...
    $('.btns li a').on('click focusin',function(e){
        e.preventDefault();
        const imgSrc = $(this).attr('href');
        $('#gallery .showBox').css({backgroundImage:`url(${imgSrc})`});
        const cur = $(this).parent().index()+1;
        console.log(cur);
        $('.controls div strong').text(cur);
        $('.btns li a').css({
            color : '#000'
        })
        $(this).css({
    ...
    /*
        미션2
        1. controls 안에 span에는 현재 이미지의 전체 갯수를 스크립트로 적용하고
        2. strong은 버튼을 클릭할 때마다 현재 이미지의 순번을 출력
        3. btns li의 a태그는 현재 클릭한 버튼만 글자색을 흰색으로 활성화
        4. 나머지버튼들은 원래 색상으로 비활성화 유지
    */
    const total = $('.controls .btns li').length;
    $('.controls span').text(total);
